...
metaDescription: 'Integrate Greptile AI-powered agent with Slack to get instant answers about your codebase. Boost developer productivity with context-aware code insights directly in your team communications.'
---

You can interact with Greptile's Slackbot by:
1. Directly messaging the Agent
2. Invoking the Agent in a channel by mentioning `@greptile`
...
Click [here](https://app.greptile.com/login) to install the Slackbot. Once you have installed the agent, you can use the same link to configure the agent. 

Configuration includes:
1. Setting up triggers, for example `@mentions`, or all messages in a specific `#channel`.
2. Setting reference repositories, so the agent can provide context-aware answers.
3. *Optional*: Providing "special instructions" for the agent to follow, for example, "Answer in 1-2 sentences only".
