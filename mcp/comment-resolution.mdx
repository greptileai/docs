...
<Note>
The MCP server is currently in development. The setup instructions below describe the planned integration process.
</Note>
...
<Steps>
    ...
    <Step title="Add Greptile MCP Server">
        ...
        ```bash
        claude mcp add --transport http greptile https://api.greptile.com/v2/mcp \
...
```json
{
  "servers": {
    "greptile": {
      "transport": "http",
      "url": "https://api.greptile.com/v2/mcp",
      "headers": {
...
<Steps>
    ...
    <Step title="Ask for Comments">
        Use these prompts to retrieve comments:
        - "Show me all unaddressed Greptile comments for this PR"
        - "What review issues need to be fixed in this file?"
        - "Get Greptile feedback for the current pull request"
    </Step>
    <Step title="Review Comment Details">
        The IDE will display:
        - Comment text and suggested fixes
        - File location and line numbers
        - Related custom context patterns
